...
stage/production은 별도 runner가 아니라 일반 `docker compose --env-file .env -f docker-compose.{stage,prod}.yaml up -d --build --wait --wait-timeout 180`에서 자동 적용합니다. Compose는 MySQL healthcheck, `jungol-migrator`의 성공 종료(`Exited (0)`), 그 뒤 앱 시작 순서를 강제합니다. 실패하면 Compose가 nonzero로 끝나며 종속 앱은 시작하지 않습니다. `docker compose ... ps`와 `docker compose ... logs jungol-migrator`로 상태를 확인합니다. dev Compose에는 migrator가 없으므로 스키마 준비는 개발자 책임입니다.
