...
test("Given migration 005 schema When checking required tables Then uses its exact application table set", () => {
...
test("Given a health repository When checking schema migration Then requires migration 005", async () => {
  ...
  expect(migrationValues).toEqual([5]);
});
