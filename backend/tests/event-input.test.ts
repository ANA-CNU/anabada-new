...
test("update parses the numeric-array contract", () => {
  expect(
    parseEventInput({ ...valid, problems: [1000, 2000] }, "update")?.problems,
  ).toEqual([1000, 2000]);
});
test("update permits removing all problems", () => {
  expect(
    parseEventInput({ ...valid, problems: [] }, "update")?.problems,
  ).toEqual([]);
});
test.each(
  [
    ...
    { ...valid, title: "" },
    { ...valid, begin: "wrong" },
    { ...valid, end: "2020-01-01" },
    { ...valid, problems: [] },
    { ...valid, problems: ["1000"] },
    { ...valid, problems: [1.5] },
  ].map((input) => ({ input })),
)("malformed create input is rejected: %j", ({ input }) => {
  expect(parseEventInput(input, "create")).toBeNull();
});
test.each(["1000,invalid", "1000,", "1.5", "-1", "0", "Infinity"])(
  "legacy string update problems are rejected: %s",
  (problems) => {
    expect(parseEventInput({ ...valid, problems }, "update")).toBeNull();
  },
);
test.each(["0", "-1", "1junk", "1.5", "", "9007199254740992"])(
  "malformed event IDs are rejected: %s",
  (id) => {
